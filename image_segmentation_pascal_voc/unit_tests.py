...
from image_segmentation_pascal_voc.data import GoldVOCSegmentation

GoldVOCSegmentation(".data", download=True)


class TestGoldVOCSegmentation:
    def test_simple_usage(self):
        dataset = GoldVOCSegmentation(".data")
    ...
    def test_with_remove_ratio(self):
        remove_ratio = 0.2
        first_dataset = GoldVOCSegmentation(".data", remove_ratio=remove_ratio)
        expected_length = int(1464 * (1 - remove_ratio))
        assert len(first_dataset) == expected_length

        second_dataset = GoldVOCSegmentation(".data", remove_ratio=remove_ratio)
    ...
    def test_with_duplicate(self):
        ...
        first_dataset = GoldVOCSegmentation(
            ".data",
            remove_ratio=0.98,
            duplicate_table_path=duplicate_table_path,
            drop_duplicate_table=drop_duplicate_table,
        ...
        second_dataset = GoldVOCSegmentation(
            ".data",
            remove_ratio=0.98,
            duplicate_table_path=duplicate_table_path,
            drop_duplicate_table=drop_duplicate_table,
